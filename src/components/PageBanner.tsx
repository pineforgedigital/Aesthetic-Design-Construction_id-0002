...
export default function PageBanner({ title, subtitle, badge, className = "py-32" }: PageBannerProps) {
  return (
    <section className={`bg-primary-contrast text-primary-base px-6 relative overflow-hidden ${className}`}>
      ...
      {/* Decorative Leaves */}
      <div className="absolute -left-16 -top-16 opacity-80 mix-blend-multiply pointer-events-none rotate-45 scale-125 md:scale-150 contrast-125 saturate-150">
        <Image src="/leaf.jpg" alt="" width={400} height={400} className="object-contain" priority unoptimized />
      </div>
      <div className="absolute -right-16 -bottom-16 opacity-80 mix-blend-multiply pointer-events-none -rotate-12 scale-125 md:scale-150 contrast-125 saturate-150">
